...
def beeswarm(shap_values, max_display=10, order=Explanation.abs.mean(0),
    ...
    if str(type(features)) == "<class 'pandas.core.frame.DataFrame'>":
        ...
        idx2cat = features.dtypes.astype(str).isin(
            ["object", "category"]).tolist()
    ...
    if feature_names is None:
        feature_names = np.array([labels['FEATURE'] % str(i)
    ...
    if len(values.shape) == 3:
        ...
        # np.argsort(-np.abs(values.sum(1)).sum(0))
        interaction_sort_inds = order
        ...
        proj_values = values[:, interaction_sort_inds[0],
                             interaction_sort_inds]
        proj_values[:, 1:] *= 2  # because off diag effects are split in half
        beeswarm(
            proj_values, features[:,
                                  interaction_sort_inds] if features is not None else None,
        ...
        pl.title(shorten_text(
            feature_names[interaction_sort_inds[0]], title_length_limit))
    ...
    for pos, inds in enumerate(orig_inds):
        if len(inds) == 1:
            ...
            feature_names_new.append(
                " + ".join([feature_names[i] for i in inds]))
        else:
    ...
    if num_features < len(values[0]) and sum_bottom_features:
        num_cut = np.sum([len(orig_inds[feature_order[i]])
...
def summary_legacy(shap_values, features=None, feature_names=None, max_display=None, plot_type=None,
    ...
    if isinstance(shap_values, list):
        ...
        if plot_type is None:
            plot_type = "dot"  # default for single output explanations
        assert len(
            shap_values.shape) != 1, "Summary plots need a matrix of shap_values, not a vector."
    ...
    if plot_type == "dot":
        for pos, i in enumerate(feature_order):
            pl.axhline(y=pos, color="#cccccc", lw=0.5,
                       dashes=(1, 5), zorder=-1)
            shaps = shap_values[:, i]
        ...
        if features is not None:
            ...
            for pos, i in enumerate(feature_order):
                ...
                xs = np.linspace(np.min(shaps) - rng * 0.2,
                                 np.max(shaps) + rng * 0.2, 100)
                if np.std(shaps) < (global_high - global_low) / 100:
                    ds = gaussian_kde(
                        shaps + np.random.randn(len(shaps)) * (global_high - global_low) / 100)(xs)
                else:
        ...
        for pos, ind in enumerate(feature_order):
            ...
            for i in range(nbins):
                ...
                ys[i, :] = gaussian_kde(
                    shaps + np.random.normal(loc=0, scale=0.001, size=shaps.shape[0]))(x_points)
        ...
        if class_inds is None:
            class_inds = np.argsort([-np.abs(shap_values[i]).mean()
    ...
    if color_bar and features is not None and plot_type != "bar" and \
        ...
        import matplotlib.cm as cm
        m = cm.ScalarMappable(cmap=cmap if plot_type !=
                              "layered_violin" else pl.get_cmap(color))
        m.set_array([0, 1])
        cb = pl.colorbar(m, ticks=[0, 1], aspect=1000)
        cb.set_ticklabels([labels['FEATURE_VALUE_LOW'],
                          labels['FEATURE_VALUE_HIGH']])
